...
def test_cursor_helpers_round_trip_values() -> None:
    pagination_schema = _import_phase_f_module("app.schemas.pagination")
    encode_cursor = _get_callable(pagination_schema, "encode_cursor")
    decode_cursor = _get_callable(pagination_schema, "decode_cursor")
    ...
    decoded = tuple(cast(tuple[str, ...], decode_cursor(encoded)))
...
def test_pagination_service_module_exposes_apply_cursor_limit_helper() -> None:
    ...
    apply_cursor_limit = _get_callable(pagination_service, "apply_cursor_limit")
...
def _seed_activity_types_for_pagination() -> tuple[object, list[tuple[datetime, str]]]:
    engine = create_engine("sqlite://")
    SQLModel.metadata.create_all(engine, tables=[ActivityType.__table__])
...
def test_apply_cursor_limit_first_page_includes_lookahead_row() -> None:
    ...
    with Session(engine) as session:
        ...
        page_rows = list(session.exec(cast(object, page_statement)))
...
def test_apply_cursor_limit_follow_up_page_uses_cursor() -> None:
    ...
    cursor = cast(
        str,
        encode_cursor([ordered_values[1][0].isoformat(), ordered_values[1][1]]),
    )
    with Session(engine) as session:
        statement = select(ActivityType)
        page_statement = apply_cursor_limit(
...
def test_apply_cursor_limit_returns_empty_page_when_cursor_is_at_end() -> None:
    ...
    end_cursor = cast(str, encode_cursor([final_row[0].isoformat(), final_row[1]]))
    with Session(engine) as session:
        statement = select(ActivityType)
        page_statement = apply_cursor_limit(
            statement,
...
def test_apply_cursor_limit_last_page_has_no_extra_row() -> None:
    ...
    engine, ordered_values = _seed_activity_types_for_pagination()
    cursor_before_last = cast(
        str,
        encode_cursor([ordered_values[3][0].isoformat(), ordered_values[3][1]]),
    )
    with Session(engine) as session:
        statement = select(ActivityType)
        page_statement = apply_cursor_limit(
